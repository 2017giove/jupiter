...
peak Ba_fit(TCanvas* c40, TH1D* h1, std::string savepath, mySetting* st, int PMTid);
void Ba_getPeak(const char* src_name, int PMTid, char* wheretosave) ;
void Ba_fitall(const char* src_name) ;
...
void Ba_fitall(const char* src_name) {
    ...
    for (int j = 0; j < st.Nchan; j++) {
        ...
        printf("\nFilename iniziale %s \n>> Salvato in %s\n", src_name, temp2);
        Ba_getPeak(src_name, PMTid, temp2);
    }
...
void Ba_getPeak(const char* src_name, int PMTid, char* wheretosave) {
    ...
    strcpy(fileOUT, appendToRootFilename(sorgente_file->GetName(), "bafit").c_str());
    ...
    sprintf(timgnames[0], "bafit");
    sprintf(timgnames[1], "bbafit");
...
peak Ba_fit(TCanvas* c40, TH1D* h1, std::string savepath, mySetting* st, int PMTid) {
    ...
    fsrc->SetParName(10, "GaussAmp3");
    fsrc->SetParName(11, "Peak3");
    fsrc->SetParName(12, "sigma3");
    ...
    float xmid = Xmax / ENERGY_BARIO_MAX * ENERGY_BARIO_MID;

    fsrc->SetParLimits(4, Ymax / 2, Ymax); //OK!
    fsrc->SetParLimits(5, xmid * 0.9, xmid * 1.1);
    fsrc->SetParLimits(6, sigma * 0.7, sigma * 1.3);


    float xmin = Xmax / ENERGY_BARIO_MAX * ENERGY_BARIO_MIN;

    fsrc->SetParLimits(7, Ymax / 2, Ymax); //OK!
    fsrc->SetParLimits(8, xmin * 0.9, xmin * 1.1);
    fsrc->SetParLimits(9, sigma * 0.7, sigma * 2);


    fsrc->SetParLimits(10, Ymax / 2, Ymax*2); //OK!
    fsrc->SetParLimits(11, 0, xmin);
    fsrc->SetParLimits(12, 0, sigma);

    fsrc->SetParLimits(13, Ymax * 0.8, Ymax * 1.2); //OK!
    fsrc->SetParLimits(14, Xmax * 0.9, Xmax * 1.1);
    fsrc->SetParLimits(15, sigma * 0.5, sigma * 1.5);
    ...
    fsrc->SetParameter(5, xmid);
    fsrc->SetParameter(6, sigma);

    fsrc->SetParameter(7, Ymax * 0.65);
    fsrc->SetParameter(8, xmin);
    fsrc->SetParameter(9, sigma);

    fsrc->SetParameter(10, Ymax);
    fsrc->SetParameter(11, xmin / 2);
    fsrc->SetParameter(12, sigma);

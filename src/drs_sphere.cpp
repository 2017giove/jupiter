...
int main(int argc, char* argv[]){
	...
	cout << "          PMT      : ID number of the chrystal" << endl;
        ...
        cout << " Example: drs_sphere lollo 300 0 1200 -25.0 neg 1 1500 500" << endl;
        return 0;
    }
  ...
  float Voltage = atof(argv[8]);
  int PMT = atoi(argv[9]);
  char* DATE = asctime(localtime(&Current_Time))/*argv[10]*/;
  //char *DATE = "podaspodasp";
  cout << DATE << endl;
  bool chanToPlot[128];
  ...
  if (argc==8) {
    ...
    for (int i=0; i<MAXCH; i++) chanToPlot[i]=false;

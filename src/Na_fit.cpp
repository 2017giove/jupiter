...
peak Na_fit(TCanvas* c40, TH1D* h1, std::string savepath, mySetting* st, int PMTid);
void Na_getPeak(const char* src_name, int PMTid, char* wheretosave);
void Na_fitall(const char* src_name);
...
void Na_fitall(const char* src_name) {
    ...
    for (int j = 0; j < st.Nchan; j++) {
        ...
        printf("\nFilename iniziale %s \n>> Salvato in %s\n", src_name, temp2);
        Na_getPeak(src_name, PMTid, temp2);
    }
...
void Na_getPeak(const char* src_name, int PMTid, char* wheretosave) {
    ...
    strcpy(fileOUT, appendToRootFilename(sorgente_file->GetName(), "nafit").c_str());
    ...
    sprintf(timgnames[0], "nafit");
    sprintf(timgnames[1], "nnafit");
...
peak Na_fit(TCanvas* c40, TH1D* h1, std::string savepath, mySetting* st, int PMTid) {
    ...
    h1->Rebin(4);
//    for (int sniper = 0; sniper < 3; sniper++) {
//
//
//        nBins = h1->GetSize() - 2;
...
//    float Ymax = h1->GetBinContent(maxBin);
    ...
    float xmin = Xmax / ENERGY_SODIO_MAX * ENERGY_SODIO_MIN;
    ...
    float startfitpoint = 5; // da cambiare in caso di nuove prese dati
    ...
    float FDCompton = Xmax * (1 - 1 / (1 + 2 * ENERGY_SODIO_MAX / MASS_ELECTRON));
    float FDCompton2 = xmin * (1 - 1 / (1 + 2 * ENERGY_SODIO_MIN / MASS_ELECTRON));
